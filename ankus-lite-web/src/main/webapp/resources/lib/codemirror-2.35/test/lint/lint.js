...
"break case catch continue debugger default delete do else false finally for function if in\
 ...
 volatile double import public const".split(" ").forEach(function (word) {
        reserved[word] = true;
    });
...
function walk(ast, scope) {
    ...
    function sub(ast) {
        if (ast) walk(ast, scope);
    }

    function subn(array) {
        if (array) array.forEach(sub);
    }
    ...
    if (tp == "block" || tp == "splice" || tp == "toplevel" || tp == "array") {
        ...
        ast[1].forEach(function (def) {
            scope.cur[def[0]] = true;
            if (def[1]) sub(def[1]);
        });
    } else if (tp == "try") {
        subn(ast[1]);
        if (ast[2]) {
            scope.cur[ast[2][0]] = true;
            subn(ast[2][1]);
        }
        subn(ast[3]);
        ...
        ast[2].forEach(function (arg) {
            nscope.cur[arg] = true;
        });
        ast[3].forEach(function (ast) {
            walk(ast, nscope);
        });
