(function() {
  ...
  function styles(el, selectorRemap, document) {
    var css = "";
...
//    var image = new Image();
    ...
    var aa = function() {
    	...
    	if (flag) {
	    	...
	    	var yPos = 0;
	    	for (var i=0; i<images.length; i++) {
	    		context.drawImage(images[i], 0, yPos);
	    		yPos += images[i].height;
	    	}	    	
	    	
	    	if (options.callback) {
	    		options.callback(canvas.toDataURL('image/png'));
...
//    out$.svgAsDataUri(el[0], options, function(uri) {
...
//      timage.src = uri;
//    });
//    out$.svgAsDataUri(el[1], options, function(uri) {
...
//    });
    ...
    for (var i=0; i<el.length; i++) {
    	out$.svgAsDataUri(el[i], options, function(uri) {
    		images[i].onload = function() {
    			flags[this.getAttribute('data-idx')] = true;
    			aa();
    		}
    		images[i].src = uri;
